...
export const createProductSchema = zod.object({
  ...
  storeId: zod.string().regex(/^[a-fA-F0-9]{24}$/, "Invalid storeId").optional(),

  // images are sent as multipart files (image[0], image[1], ...)
}).strict();

export const updateProductSchema = zod.object({
  name: zod.string().min(1).trim().optional(),
  description: zod.string().optional(),
  sellingPrice: zod.number().nonnegative().optional(),
  storeId: zod.string().regex(/^[a-fA-F0-9]{24}$/, "Invalid storeId").optional(),
}).strict();
